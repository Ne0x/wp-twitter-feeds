<?php class wptt_TwitterTweets extends WP_Widget{
	...
	function sanitize_links($tweet) {
		...
		$text = preg_replace('/[@]+([A-Za-z0-9-_äÄöÖåÅ]+)/', '<a href="http://twitter.com/$1" target="_blank" rel="nofollow">@$1</a>', $text );
		$text = preg_replace('/[#]+([A-Za-z0-9-_äÄöÖåÅ]+)/', '<a href="http://twitter.com/search?q=%23$1" target="_blank" rel="nofollow">$0</a>', $text );
		return $text;
//'/[#]+([A-Za-z0-9-_äÄöÖåÅ]+)/'
	}
